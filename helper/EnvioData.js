...
const EnvioData =  async (Norecibo = "", Data = "", Key = "") => {
 ...
 // console.log( 'NO RECIBO ', Norecibo);
  ...
  try {
    ...
    if (resp.status === 201) {
       sql = "update tblpagos p set p.sincronizadocontable = 1 where p.norecibo = ?"
       
       parametros.push(Norecibo)
       try {
        const pagos = await consulta(sql , parametros);
          
            sql = "INSERT INTO TBLTRANSACCIONESCIERRE ( NORECIBO, CODIGO, DETALLE) VALUES ( ?, ?, ?);"
            parametros.push("200")
